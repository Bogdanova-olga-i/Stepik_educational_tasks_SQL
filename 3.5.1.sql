...
SELECT CONCAT(LEFT(CONCAT_WS(' ', module_id, module_name), 16), '...') AS Модуль,
    CONCAT(LEFT(CONCAT_WS('.', module_id, CONCAT_WS(' ', lesson_position, lesson_name)), 16), '...') AS Урок,
    CONCAT_WS('.', module_id, lesson_position, CONCAT_WS(' ', step_position, step_name)) AS Шаг
FROM module
